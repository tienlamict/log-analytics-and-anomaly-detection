...
```bash
INIT=$(node "D:/Project/Golang/log-analytics-and-anomaly-detection/.claude/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "D:/Project/Golang/log-analytics-and-anomaly-detection/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
